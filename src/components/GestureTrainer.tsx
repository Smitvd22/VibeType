...
import { useGestureTraining } from "@/hooks/useGestureTraining";
...
export function GestureTrainer({ currentLandmarks, onDone }: GestureTrainerProps) {
  const { phase, countdown, captureGesture } = useGestureTraining(currentLandmarks);
  ...
  const handleCapture = () => {
    ...
    setError(null);
    captureGesture(name, emoji, onDone, setError);
  };

  return (
    <div className="flex flex-col gap-4 p-5 glass rounded-2xl border border-white/5 shadow-xl">
      <h3 className="font-bold text-white text-lg">Train New Gesture</h3>
      
      <div className="flex gap-3">
        <input 
          type="text" 
          placeholder="Gesture Name (e.g. Spock)" 
          value={name} 
      ...
      <p className="text-xs text-zinc-400 text-center mt-1">
        Hold your hand clearly in frame and click Capture.
      </p>
    </div>
  );
}
